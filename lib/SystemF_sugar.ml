...
let rec desugar_typ (decls : decls) (typ : typ) : F.typ = match typ with
  ...
  | TAbbrv (x, ts) -> 
    (match List.assoc x decls with
    | TypAbbrv (xs, t) ->
      let fresh_xs = List.map Identifier.fresh_from xs in
      let fresh_t = 
        List.fold_right2 rename_tid xs fresh_xs (desugar_typ decls t) in
      List.fold_right2 
        tsubst'
        fresh_xs (List.map (desugar_typ decls) ts)
        fresh_t)
...
let rec desugar_exp (decls : decls) (exp : exp) = match exp with
  ...
  | TypApp (p, e, t) -> F.TypApp (p, desugar_exp decls e, desugar_typ decls t)
